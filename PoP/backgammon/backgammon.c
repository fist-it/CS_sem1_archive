    ...
    if (game->board[6 + i] < 0) {
      ...
      if (-1 * game->board[i + 6] > 5) {
        if (-1 * game->board[i + 6] < 15) {
          mvprintw(UP + 6, BAR_L - 3 - (i * 3), "+%d",
                   -(5 + game->board[i + 6]));
        } else {
          mvprintw(UP + 6, BAR_L - 4 - (i * 3), "+%d",
                   -(5 + game->board[i + 6]));
        }
      }
    }
    ...
    if (game->board[12 + i] > 0) {
      ...
      if (game->board[i + 12] > 5) {
        if (game->board[i + 12] < 15) {
          mvprintw(DOWN - 6, LEFT + 2 + (i * 3), "+%d",
                   game->board[i + 12] - 5);
        } else {
          mvprintw(DOWN - 6, LEFT + 1 + (i * 3), "+%d",
                   game->board[i + 12] - 5);
        }
      }
    }
    ...
    if (game->board[18 + i] < 0) {
      ...
      if (-1 * game->board[i + 18] > 5) {
        if (-1 * game->board[i + 18] < 15) {
          mvprintw(DOWN - 6, BAR_R + 2 + (i * 3), "+%d",
                   -(5 + game->board[i + 18]));
        } else {
          mvprintw(DOWN - 6, BAR_R + 1 + (i * 3), "+%d",
                   -(5 + game->board[i + 18]));
        }
      }
    }
...
void invalid_move() { mvprintw(WARNING_Y, WARNING_X, "Invalid move!"); }
...
void win(int person, backgammon_t *game) {
  clear();
  if (!person) {
    mvprintw(0, 0, "White wins with oponent having %d pieces at home!",
             game->at_home[1]);
  } else {
    mvprintw(0, 0, "Black wins with oponent having %d pieces at home!",
             game->at_home[0]);
  }
...
void end_phase_white(backgammon_t *game, int from, int dice) {
  if (game->board[from] > 0) {
    if (from + dice >= BOARD_SIZE) {
      ...
      refresh_board(game);
      mvprintw(PLAYER_INFO_Y + 1, PLAYER_INFO_X, "Moved from %d to %d",
               from + 1, from + dice + 1);
    }
  } else {
    invalid_move();
...
void move_white(backgammon_t *game, int dice) {
  eval_win(game);
  clear_player_info();
  mvprintw(PLAYER_INFO_Y, PLAYER_INFO_X,
           "Choose your move by index (moving by %d): ", dice);
  int from = get_number();
  if (game->home[0] >= 15) {
    end_phase_white(game, from, dice);
  } else {
    if (game->board[from] > 0 && game->board[from + dice] >= 0 &&
        from + dice < BOARD_SIZE && from >= 0) {
      game->board[from]--;
      game->board[from + dice]++;
      refresh_board(game);
      mvprintw(PLAYER_INFO_Y + 1, PLAYER_INFO_X, "Moved from %d to %d",
    ...
    // NIE WYMUSZONE BICIE
    /*
    else if(game->board[from] > 0 && game->board[from+dice] == -1 && from+dice <
    BOARD_SIZE && from >= 0){ hit_white_to_black(game, from, from+dice);
    }
    */
...
void move_black(backgammon_t *game, int dice) {
  ...
  if (game->home[1] >= 15) {
    ...
    0 && from < BOARD_SIZE){ hit_black_to_white(game, from, from-dice);
    }
    */
    else {
      invalid_move();
...
void check_comeback_white(backgammon_t *game) {
  if (game->board[game->dice[0] - 1] == 0 ||
      game->board[game->dice[1] - 1] == -1) {
    comeback_white_dice1(game);
  } else if (game->board[game->dice[1] - 1] == 0 ||
             game->board[game->dice[1] - 1] == -1) {
    comeback_white_dice1(game);
  }
  mvprintw(BAR_DEBUG_Y, BAR_DEBUG_X, "Bar 1 = %d", game->bar[0]);
...
void comeback_black_dice1(backgammon_t *game) {
  clear_info();
  mvprintw(COMEBACK_Y, COMEBACK_X, "Black came back at %d!",
           BOARD_SIZE - game->dice[0] + 1);
...
void check_comeback_black(backgammon_t *game) {
  if (game->board[BOARD_SIZE - game->dice[0]] == 0 ||
      game->board[BOARD_SIZE - game->dice[1]] == 1) {
    comeback_black_dice1(game);
  } else if (game->board[BOARD_SIZE - game->dice[1]] == 0 ||
             game->board[BOARD_SIZE - game->dice[1]] == 1) {
